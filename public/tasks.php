    ...
    try {
        ...
        if (PRINTER_ENABLED && isset($_POST['print_task'])) {
            if (Printer::useBrowserPrint()) {
                // Store task ID in session for browser print
                $_SESSION['print_task_id'] = $task->getId();
                $_SESSION['success'] = 'Task created! Opening print dialog...';
            } else {
    ...
    try {
        ...
        if ($task->get('user_id') == $user->getId()) {
            if (Printer::useBrowserPrint()) {
                ...
                $_SESSION['success'] = 'Opening print dialog...';
            } else {
                // Direct ESC/POS printing
                $printer = new Printer();
                $printer->printTaskReceipt($task);
